...
void Marble::startCenterCamera()
{
    if (mVelocity.y * mVelocity.y + mVelocity.x * mVelocity.x + mVelocity.z * mVelocity.z >= 9.0)
    {
        ...
        mRadsLeftToCenter = mAtan2(mVelocity.x, mVelocity.y) - mAtan2(motionDir.x, motionDir.y);
...
#else
    double dt = ClientProcessList::get()->getLastDelta();
#endif
...
void Marble::cameraLookAtPt(const Point3F& pt)
{
    ...
    if (deltaRot.len() >= 0.0001)
        this->mMouseX = mAtan2(deltaRot.x, deltaRot.y);
}
...
#else
    float backDelta = ClientProcessList::get()->getLastDelta();
#endif
...
void Marble::setPlatformsForCamera(const Point3F& marblePos, const Point3F& startCam, const Point3F& endCam)
{
    ...
    camBox.minExtents = marblePos + camBox.minExtents;
    camBox.maxExtents = marblePos + camBox.maxExtents;

    camBox.minExtents.setMin(startCam - 0.25f);
    camBox.maxExtents.setMax(startCam + 0.25f);
    camBox.minExtents.setMin(endCam - 0.25f);
    camBox.maxExtents.setMax(endCam + 0.25f);
    
    camBox.minExtents -= 0.25f;
    camBox.maxExtents += 0.25f;

#ifdef MB_ULTRA_PREVIEWS
    F32 delta = getCurrentClientProcessList()->getLastDelta();
#else
    F32 delta = ClientProcessList::get()->getLastDelta();
#endif
...
void Marble::getCameraTransform(F32* pos, MatrixF* mat)
{
    ...
    Box3F testBox;
    if (mCameraInit)
        testBox.maxExtents = mLastCamPos;
    else
        testBox.maxExtents = endPos;
    testBox.minExtents = testBox.maxExtents;
    testBox.minExtents.setMin(endPos);
    testBox.minExtents.setMin(position);
    testBox.maxExtents.setMax(endPos);
    testBox.maxExtents.setMax(position);
...
DefineEngineMethod(Marble, cameraLookAtPt, void, (Point3F point),, "(point)")
{
    object->cameraLookAtPt(point);
}
