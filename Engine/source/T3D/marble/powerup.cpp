...
void PowerUpData::initPersistFields()
{
    ...
    addField("emitter", TYPEID<ParticleEmitterData>(), Offset(emitter, PowerUpData), MaxPowerUps);
    ...
    addField("image", TYPEID<GameBaseData>(), Offset(image, PowerUpData), MaxPowerUps);
...
bool PowerUpData::preload(bool server, String& errorStr)
{
    bool result = Parent::preload(server, errorStr);
